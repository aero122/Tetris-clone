...
procedure TForm1.Button2Click(Sender: TObject);//Создаём фигуру
begin
  figure.vid:=1;
  figure.left:=5;
  figure.top:=1;
...
begin
  ...
  case key of
    65:if figure.left>1 then figure.left:=figure.left-1;//Смещение влево при нажатии A
    68:if figure.left<10 then figure.left:=figure.left+1;//Смещение вправо при нажатии D
  end;
...
procedure TForm1.Draw();//Процедура переотрисовки
var
  i: Integer;
  j: Integer;
begin
...
procedure TForm1.FormShow(Sender: TObject);//При показе формы отрисовываем белый фон
begin
  Image1.Canvas.Brush.Color:=clWhite;
  Image1.Canvas.FillRect(Image1.BoundsRect);
end;
